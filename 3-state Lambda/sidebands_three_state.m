...
j=0;
for spacing=2.5:2.5:22.5

    i=0;
    
    j=j+1;
for Mod_depth=0.001:1:5.1
    i=i+1;
 
    Omegas=[];
    
for Rabi_ae=0:0.7:40
    ...
    leng=round(length(solution.y(9,:))/5);
    Omegas=[Omegas;Rabi_ae,mean(solution.y(9,end-leng:end)),mean(solution.y(5,end-leng:end)),mean(solution.y(1,end-leng:end))];
...
figure(1)
subplot(3,3,j)
plot(Omegas(:,1)/(Gamma_a+Gamma_b),Omegas(:,2))
xlabel('Rabi rate [\Gamma]')
ylabel('\rho_{ee}')
ylim([0 0.1])
title(['Spacing ' num2str(spacing/(Gamma_a+Gamma_b)) '\Gamma'])
drawnow
if Mod_depth==0.001
    legendInfo1{i}=['\beta=' num2str(0)];
else
    legendInfo1{i}=['\beta=' num2str(Mod_depth-0.001)];
end
hold on

figure(2)
subplot(3,3,j)
plot(Omegas(:,1)/(Gamma_a+Gamma_b),Omegas(:,3))
xlabel('Rabi rate [\Gamma]')
ylabel('\rho_{bb}')
ylim([0 1])
title(['Spacing ' num2str(spacing/(Gamma_a+Gamma_b)) '\Gamma'])
drawnow
if Mod_depth==0.001
    legendInfo2{i}=['\beta=' num2str(0)];
else
    legendInfo2{i}=['\beta=' num2str(Mod_depth-0.001)];
end
hold on

figure(3)
subplot(3,3,j)
plot(Omegas(:,1)/(Gamma_a+Gamma_b),Omegas(:,4))
xlabel('Rabi rate [\Gamma]')
ylabel('\rho_{aa}')
ylim([0 1])
title(['Spacing ' num2str(spacing/(Gamma_a+Gamma_b)) '\Gamma'])
drawnow
if Mod_depth==0.001
    legendInfo3{i}=['\beta=' num2str(0)];
else
    legendInfo3{i}=['\beta=' num2str(Mod_depth-0.001)];
end
hold on
